    ...
    <header className="bg-white shadow-sm p-4 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
                <div className="w-10 h-10 rounded-full bg-pink-500 text-white flex items-center justify-center font-bold">JW</div>
...
const Dashboard = ({ navigate, projects, openCreateProjectModal }) => {
    ...
    return (
        <div className="bg-gray-50 flex-1 overflow-y-auto"><Header title="Home" /><main className="p-8"><div className="flex justify-between items-center mb-8"><div><p className="text-gray-500">{formattedDate}</p><h1 className="text-3xl font-bold text-gray-800">Good afternoon, Justin</h1></div><button className="flex items-center px-4 py-2 border border-gray-300 rounded-lg text-gray-600 hover:bg-gray-100"><Icon path="M10.343 3.94c.09-.542.56-1.007 1.11-1.227a4.5 4.5 0 112.863 8.43M16.5 9a4.5 4.5 0 11-8.43 2.863m-1.007 1.11a4.5 4.5 0 11-2.863-8.43" className="w-5 h-5 mr-2" />Customize</button></div><div className="grid grid-cols-1 lg:grid-cols-3 gap-8"><div className="lg:col-span-2"><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><OverviewCard title="Recent Projects" icon={<Icon path="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />}>{projects.length === 0 ? <EmptyState message="No active projects" buttonText="Create Project" onButtonClick={openCreateProjectModal} /> : <div>{projects.slice(0, 3).map(p => <div key={p.id} className="p-3 bg-gray-50 rounded-lg mb-2 shadow-sm"><p className="font-bold">{p.name}</p><p className="text-sm text-gray-600">{p.description}</p></div>)}<button onClick={openCreateProjectModal} className="mt-4 text-blue-500 font-semibold hover:underline">+ Create Project</button></div>}</OverviewCard><OverviewCard title="My Tasks" icon={<Icon path="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />}><EmptyState message="No tasks assigned" buttonText="View Task Boards" onButtonClick={() => navigate('#/task-boards')} /></OverviewCard><div className="md:col-span-2"><OverviewCard title="Recent Activity" icon={<Icon path="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />}><EmptyState message="No recent activity" /></OverviewCard></div></div></div><div><OverviewCard title="My Team" icon={<Icon path="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m-7.5-2.962a4.5 4.5 0 119 0 4.5 4.5 0 01-9 0zm0 0a4.5 4.5 0 10-9 0 4.5 4.5 0 009 0z" />}><div className="flex flex-col space-y-4"><div className="flex items-center"><div className="w-12 h-12 rounded-full bg-pink-500 text-white flex items-center justify-center font-bold text-xl">JW</div><div className="ml-4"><p className="font-semibold">Justin Wilson</p><p className="text-sm text-gray-500">Owner</p></div></div><div className="flex items-center"><div className="w-12 h-12 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center"><Icon path="M12 4.5v15m7.5-7.5h-15" className="w-6 h-6 text-gray-400" /></div><div className="ml-4"><p className="font-semibold">Invite User</p></div></div></div></OverviewCard></div></div></main></div>
    );
};
...
const TaskBoard = () => {
    ...
    const handleTaskCreate = (task) => setTasks(prev => [...prev, { ...task, id: Date.now() }]);
    ...
    return (
        <div className="bg-gray-50 flex-1 overflow-y-auto"><Header title="Task Boards" /><main className="p-8"><div className="flex justify-between items-center mb-8"><h1 className="text-3xl font-bold">Collaborate on tasks</h1><button onClick={() => setIsModalOpen(true)} className="bg-pink-500 hover:bg-pink-600 text-white font-bold py-2 px-4 rounded-lg flex items-center"><Icon path="M12 4.5v15m7.5-7.5h-15" className="w-5 h-5 mr-2" />New Task Board</button></div><div className="grid grid-cols-1 md:grid-cols-3 gap-8">{columns.map(status => <div key={status} className="bg-gray-100 rounded-lg p-4" onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, status)}><h3 className="font-bold text-lg text-gray-700 mb-4">{status}</h3><div className="space-y-4">{tasks.filter(t => t.status === status).map(t => <div key={t.id} draggable onDragStart={(e) => handleDragStart(e, t)} className="bg-white p-4 rounded-lg shadow-sm cursor-grab">{t.name}</div>)}</div></div>)}</div></main><CreateTaskModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onTaskCreate={handleTaskCreate} /></div>
    );
};

const UsersPage = () => {
    const [users, setUsers] = useState([{ id: 1, name: 'Justin Wilson', email: 'justinwilson@gmail.com', phone: '-', role: 'Owner', status: 'Active' }]);
    ...
    const handleUserInvite = (newUser) => setUsers(prev => [...prev, { ...newUser, id: Date.now(), status: 'Pending', phone: '-' }]);
...
const ContactsPage = () => {
    ...
    const handleContactCreate = (newContact) => setContacts(prev => [...prev, { ...newContact, id: Date.now() }]);
    return (
...
export default function App() {
  ...
  const handleProjectCreate = (project) => setProjects(prev => [...prev, { ...project, id: Date.now() }]);
  ...
  return (
    <div className="flex h-screen bg-gray-100 font-sans">
      ...
      <Router currentPath={currentPath} navigate={navigate} projects={projects} openCreateProjectModal={() => setIsProjectModalOpen(true)} />
